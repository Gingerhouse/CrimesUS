...
def load_crime_data():
   ...
   crime_corr = pd.read_csv("corr.csv")
...
with tab2:
   ...
   with st.sidebar:
    crime_corr = crime_corr.reset_index(level=0)
    ...
    crime_selected_corr = crime_corr[['index', 'murders']].sort_values(by='murders', ascending=False)
   ...
   #  '2.' + filtered_corr.iloc[] + "\n" + 
   #  '3.' + filtered_corr.iloc[] + "\n" + 
   #  '4.' + filtered_corr.iloc[] + "\n" + 
   #  '5.' + filtered_corr.iloc[])
